...
export const StrategicPillarsSection = ({ data }) => {
    ...
    const disabled = content.pillars.some(
        (item) =>
            content.title === "" ||
    ...
    const updateItem = (index, key, event) => {
        const newArray = [...content.pillars];
        newArray[index][key] = event.target.value;
        updateContent({ ...content, pillars: newArray });
    };
                    ...
                    <Editor
                        ...
                        custom={
                            <div className="border border-teal-900 rounded-lg">
                                <Carousel
                                    items={content.pillars.map((item, i) => (
                                        <div
                                            key={i}
